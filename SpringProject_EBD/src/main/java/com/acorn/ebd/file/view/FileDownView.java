package com.acorn.ebd.file.view;
...
public class FileDownView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		//3. 해당 파일을 실제로 다운로드 시켜준다(응답)
	 	//다운로드 시켜주기 ... 
		String orgfname=dto.getOrgfname(); //원본 파일명
		String savefname=dto.getSavefname(); //저장된 파일명
		
		//다운로드 시켜줄 파일의 실제 경로 구성하기
		String path=request.getServletContext().getRealPath("/upload")
				+File.separator+savefname;
		...
		if(request.getHeader("User-Agent").contains("Firefox")){
			...
			encodedName=new String
				(orgfname.getBytes("utf-8"),"ISO-8859-1");
		}else{ //그외 다른 벤더사 
			encodedName=URLEncoder.encode(orgfname, "utf-8");
			//파일명에 공백이있는 경우 처리 
